...
class HotelSearchResultsPage extends Page {
    ...
    private filteredPriceMinValue(){ return $("//div[@id='slider-range-filter-priceRange-content']/div[contains(@class,'StyleUtility')][1]//p[1]"); }
    private filteredPriceMaxValue(){ return $("//div[@id='slider-range-filter-priceRange-content']/div[contains(@class,'StyleUtility')][1]//p[3]"); }
    ...
    private get hotelPriceExcludesponsored() { return $$("");}
   ...
   //#endregion
    ...
    private async listOfAllFilterItemsByType(filterName: string) {
        return await $$("//div[contains(@class,'Accordion')]//p[contains(@class,'AccordionHeader') and text()='" + filterName + "']/../..//div[contains(@class,'Body') or contains(@class,'Label')]");
    }
    private async filterHeaderByType(filterName: string) {
        return await $("//div[contains(@class,'Accordion')]//p[contains(@class,'AccordionHeader') and text()='" + filterName + "']/..");
    }

    private async filterItemByType(filterName: string, filterItem: string) {
        return await $("//div[contains(@class,'Accordion')]//p[contains(@class,'AccordionHeader') and text()='" + filterName + "']/../..//div[text()=\"" + filterItem + "\"]/..//input");
    }
   ...
   //#endregion
    ...
    async uncheckAllFilterItems(filterName: string, allFilterItem:any) {
        for (let allItem = 0; allItem < await allFilterItem.length; allItem++) {
            await this.uncheckFilterItem(filterName, await allFilterItem[allItem]);
        }
    }
    ...
    async verifyFilterLabelOnHotelCardAfterSingleFilterSelection(filterName: string,filterItem:any) {
        ...
        for (let filterIndex = 0; filterIndex < await filterItem.length; filterIndex++) {
            ...
            await this.uncheckAllFilterItems(filterName,await filterItem);
    ...
    async unSelectAllStarRating(allFilterItem:any) {
        for (let allItem = 0; allItem < allFilterItem.length; allItem++) {
            await this.unSelectStarRating(allFilterItem[allItem])
        }
    }
    ...
    async verifyStarRatingOnHotelCardAfterSingleSelection(filterName: string,filterItem:any) {
        ...
        for (let filterIndex = 0; filterIndex < await filterItem.length; filterIndex++) {
            var selectedFilterItem = [];
            await this.unSelectAllStarRating(await filterItem);
            var filterItemName = await filterItem[filterIndex];
            selectedFilterItem.push(await filterItemName);
    ...
    async waitForSearchResultsToLoad(): Promise<void> {
        try {
            await (await this.loaderIcon).waitForExist({ timeout: 4000 });
