...
{
    public class EdgeDriverDecorator : EdgeDriver, IAtWebDriver
    {
        ...
        private static string EdgeDriverDirectory => AtConfiguration.GetDriverDirectory();

        private static int DefaultPollingInterval => AtConfiguration.GetDefaultConfiguration("DefaultPollingInterval", 250);
        ...
        private static EdgeOptions Options => new EdgeOptions();

        public EdgeDriverDecorator()
          : this(EdgeDriverDirectory, Options)
        {
        }

        public EdgeDriverDecorator(DriverOptions driverOptions)
           : this(EdgeDriverDirectory, (EdgeOptions)driverOptions)
        {
        }

        private EdgeDriverDecorator(string edgeDriverDirectory, EdgeOptions options)
            : base(edgeDriverDirectory, options)
        {
        ...
        public AtWebElement FindElement(LocatorType locatorType, string locatorValue, params string[] replaceHashValues)
        {
            ...
            return new EdgeAtWebElement(element)
            {
        ...
        public ReadOnlyCollection<AtWebElement> FindElements(LocatorType locatorType, string locatorValue,
            params string[] replaceHashValues)
        {
            var edgeElements = new List<AtWebElement>();
            ...
            {
                edgeElements.Add(new EdgeAtWebElement(element)
                {
                    Selector = element?.Selector,
                    Locator = element?.Locator ?? LocatorType.ClassName,
                    LocatorValue = element?.LocatorValue,
            ...
            return edgeElements.AsReadOnly();
        }
        ...
        public ReadOnlyCollection<AtWebElement> FindElements(IWebElement parentElement, AtBy atBy)
        {
            ...
            foreach (var element in atElements)
            {
                edgeElements.Add(new ChromeAtWebElement(element)
                {
                    Selector = element?.Selector,
                    Locator = element?.Locator ?? LocatorType.ClassName,
                    LocatorValue = element?.LocatorValue,
        ...
        public void GotoUrl(string url)
        {
            ...
            {
                this.Navigate().GoToUrl("javascript:document.getElementById('invalidcert_continue').click()");
                _navigationHandler.WaitInSec(1);
            }
        }
        ...
        {
            _navigationHandler.SwitchToWindow(this, title, DefaultWaitForElement);
        }
