...
{
    public class InternetExplorerDriverDecorator : InternetExplorerDriver, IAtWebDriver
    {
        ...
        private static string IeDriverDirectory => AtConfiguration.GetDriverDirectory();
        ...
        private static InternetExplorerOptions InternetExplorerOptionsOptions
        {
            get
            {
        ...
        public InternetExplorerDriverDecorator()
            : this(IeDriverDirectory, InternetExplorerOptionsOptions)
        {
        }

        public InternetExplorerDriverDecorator(DriverOptions internetExplorerOptionsOptions)
            : this(IeDriverDirectory, (InternetExplorerOptions)internetExplorerOptionsOptions)
        {
        }

        private InternetExplorerDriverDecorator(string internetExplorerDriverDirectory, InternetExplorerOptions options)
            : base(internetExplorerDriverDirectory, options, TimeSpan.FromSeconds(DefaultHttpRequestTimeout))
        ...
        public AtWebElement FindElement(LocatorType locatorType, string locatorValue, params string[] replaceHashValues)
        {
            ...
            return new IeAtWebElement(element)
            {
        ...
        public ReadOnlyCollection<AtWebElement> FindElements(LocatorType locatorType, string locatorValue,
            params string[] replaceHashValues)
        {
            var ieElements = new List<AtWebElement>();
            ...
            {
                ieElements.Add(new IeAtWebElement(element)
                {
                    Selector = element?.Selector,
                    Locator = element?.Locator ?? LocatorType.ClassName,
                    LocatorValue = element?.LocatorValue,
      ...
      string locatorValue, params string[] replaceHashValues)
        ...
        public ReadOnlyCollection<AtWebElement> FindElements(IWebElement parentElement, AtBy atBy)
        {
            ...
            foreach (var element in atElements)
            {
                ieElements.Add(new ChromeAtWebElement(element)
                {
                    Selector = element?.Selector,
                    Locator = element?.Locator ?? LocatorType.ClassName,
                    LocatorValue = element?.LocatorValue,
        ...
        public void GotoUrl(string url)
        {
            ...
            // Incase of SSL Certificate handle it
            if (this.Title.Contains("Certificate"))
            {
                this.Navigate().GoToUrl("javascript:document.getElementById('overridelink').click()");
                _navigationHandler.WaitInSec();
            }
        }
