...
using Dnata.Automation.BDDFramework.Handlers;
...
{
    public class FirefoxDriverDecorator : FirefoxDriver, IAtWebDriver
    {
        private readonly BaseClickHandler _clickHandler;
        private readonly BaseNavigationHandler _navigationHandler;

        public bool IsDriverActive { get; set; } = true;

        public Guid InstanceId { get; } = Guid.NewGuid();

        private static string FirefoxDriverDirectory => AtConfiguration.GetDriverDirectory();

        private static int DefaultPollingInterval => AtConfiguration.GetDefaultConfiguration("DefaultPollingInterval",
            500);

        private static int DefaultHttpRequestTimeout => AtConfiguration.GetDefaultConfiguration("DefaultHttpRequestTimeOut",
            120);

        private static int DefaultJavaScriptTimeout => AtConfiguration.GetDefaultConfiguration("DefaultJavaScriptTimeout",
            120);
        ...
        private static FirefoxDriverService FirefoxDriverService
        {
            get
            {
                var firefoxDriverService = FirefoxDriverService.CreateDefaultService(FirefoxDriverDirectory);
        ...
        public FirefoxDriverDecorator()
            :this(FirefoxDriverService, FirefoxOptions)
        {

        }

        public FirefoxDriverDecorator(DriverOptions firefoxDriverOptions)
          : this(FirefoxDriverService, (FirefoxOptions)firefoxDriverOptions)
        {

        }

        private FirefoxDriverDecorator(string firefoxDriverDirectory, FirefoxOptions options)
            : base(firefoxDriverDirectory, options, TimeSpan.FromSeconds(DefaultHttpRequestTimeout))
        {
        ...
        private FirefoxDriverDecorator(FirefoxDriverService firefoxDriverService, FirefoxOptions options)
            :base(firefoxDriverService, options, TimeSpan.FromSeconds(DefaultHttpRequestTimeout))
        {
            _navigationHandler = new BaseNavigationHandler();
            Manage().Timeouts().PageLoad = TimeSpan.FromSeconds(DefaultPageLoadTimeout);
            Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(DefaultImplicitWait);
        ...
        public AtWebElement FindElement(LocatorType locatorType, string locatorValue, params string[] replaceHashValues)
        {
            ...
            return new FirefoxAtWebElement(element)
            {
        ...
        public AtWebElement FindElementByIndex(LocatorType locatorType, string locatorValue, int index, params string[] replaceHashValues)
        {
        ...
        {
            var firefoxElements = new List<AtWebElement>();
            ...
            {
                firefoxElements.Add(new FirefoxAtWebElement(element)
                {
                    Selector = element?.Selector,
                    Locator = element?.Locator ?? LocatorType.ClassName,
                    LocatorValue = element?.LocatorValue,
            ...
            return firefoxElements.AsReadOnly();
        }
       ...
       string locatorValue, params string[] replaceHashValues)
        ...
        public ReadOnlyCollection<AtWebElement> FindElements(IWebElement parentElement, AtBy atBy)
        {
            ...
            foreach (var element in atElements)
            {
                firefoxElements.Add(new ChromeAtWebElement(element)
                {
                    Selector = element?.Selector,
                    Locator = element?.Locator ?? LocatorType.ClassName,
                    LocatorValue = element?.LocatorValue,
        ...
        public void WaitForDomReady(int pollingAttempts = 240)
        {
